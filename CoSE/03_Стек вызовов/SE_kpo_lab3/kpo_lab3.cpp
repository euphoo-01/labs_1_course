...
int days_in_months[12] = { 31, 28 /*Февраль, модифицируем*/, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 };
...
int main()
{
	...
	cout << "Введенная дата: " << cur_day << "\t" << showMonth(cur_month) << "\t" << cur_year << endl;
	...
	cout << "Осталось дней до моего дня рождения: " << remaining_days_for_bday << endl;

	int cur_serial_day;
	cout << "Введите порядковый номер дня: "; cin >> cur_serial_day;
	while (cur_serial_day < 0 || cur_serial_day > 366) {
		cout << "ОШИБКА! Введите порядковый номер дня: "; cin >> cur_serial_day;
	}
