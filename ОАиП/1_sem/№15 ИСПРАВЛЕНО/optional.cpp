...
void var16_ex1() { // ПОКАЗАТЬ
	...
	for (int i{}; i < n; i++) {
		if (fabs(*(arrayPtr + i)) < fabs(minAbsoluteElement)) {
...
void var16_ex2() {
    ...
    for (int d = 0; d < n; d++) {
        int sum = 0;
        for (int i = 0; i < n - d; i++) {
            sum += cells[i][i + d]; // Элементы диагоналей выше и главной
        }
        if (sum > maxSum) {
            maxSum = sum;
        }
    ...
    for (int d = 1; d < n; d++) {
        int sum = 0;
        for (int i = 0; i < n - d; i++) {
            sum += cells[i + d][i]; // Элементы диагоналей ниже главной
        }
        if (sum > maxSum) {
            maxSum = sum;
        }
	...
	cout << "Максимум среди сумм элементов, параллельных главной диагонали: " << maxSum << endl;
} 